...
def DTSumAndPop(sourceInpt,TrgtInpt,F):
    ...
    for i in range(1,NPs):
        ...
        S[:,3:6]=np.reshape(sourceInpt[:,i],[NV,3])
        p=Pool()
        fcs=np.reshape(F,NVF*NF)
        ...
        Y=Y-A[:,NV-1].dot(c)
        AA=A_Trnsps.dot(A_Trnsps.transpose())
        b=A_Trnsps.dot(Y)
        P[:NV-1,3]=sl.spsolve(AA,b[:,0])
        P[:NV-1,4]=sl.spsolve(AA,b[:,1])
        P[:NV-1,5]=sl.spsolve(AA,b[:,2])
...
class DeformationTransferTools(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        if np.size(TrgtInpt)/len(TrgtInpt)==1: 
            DTSumAndPop(sourceInpt,TrgtInpt,F)
        else:
            DTSumAndPop(sourceInpt,TrgtInpt[:,0],F)
        return {'FINISHED'}
